...
uploadForms.forEach((form) => {
  form.addEventListener("submit", async function (e) {
    ...
    // formData.append("_csrf", getCsrfToken()); // Removed: Sent in query string instead
    ...
    try {
      // Append CSRF token to query string for multipart/form-data
      const csrfToken = getCsrfToken();
      const uploadUrl = `/members/profile/upload-documents?_csrf=${encodeURIComponent(
        csrfToken
      )}`;
...
if (profileForm) {
  profileForm.addEventListener("submit", async function (e) {
    ...
    if (
      !confirm(
        "This form can only be submitted once. Are you sure all information is correct?"
      )
    ) {
      return;
    }
