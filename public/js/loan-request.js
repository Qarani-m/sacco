...
const totalShares = parseInt(document.querySelector('.stat-value').textContent) || 0;
const maxLoanWithoutGuarantors = totalShares * 1000;
...
function updateLoanSummary() {
    ...
    document.getElementById('summaryInterest').textContent = `KSh ${interest.toLocaleString()}`;
    ...
    if (amount > maxLoanWithoutGuarantors) {
        ...
        document.getElementById('shortfallAmount').textContent = `KSh ${shortfall.toLocaleString()}`;
        document.getElementById('sharesNeeded').textContent = sharesNeeded;
        document.getElementById('amountHelp').textContent = `You need ${sharesNeeded} shares from guarantors`;
        document.getElementById('amountHelp').style.color = '#EF4444';
    } else {
        document.getElementById('guarantorSection').style.display = 'none';
        document.getElementById('amountHelp').textContent = `Your shares cover up to KSh ${maxLoanWithoutGuarantors.toLocaleString()}`;
        document.getElementById('amountHelp').style.color = '#6B7280';
    }
}

// Search for guarantors
document.getElementById('searchGuarantorsBtn').addEventListener('click', async function() {
    try {
...
function showGuarantorModal(guarantors) {
    ...
    if (guarantors.length === 0) {
        list.innerHTML = '<p class="text-secondary" style="padding: 2rem; text-align: center;">No eligible guarantors available</p>';
    } else {
        list.innerHTML = guarantors.map(g => `
            <div class="card-hover" style="padding: 1rem; margin-bottom: 1rem; display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <p style="font-weight: 600; margin-bottom: 0.25rem;">${g.full_name}</p>
                    <p class="text-secondary" style="font-size: 0.875rem;">
                        Available shares: ${g.available_shares} (KSh ${(g.available_shares * 1000).toLocaleString()})
                    </p>
                </div>
                <div style="display: flex; gap: 0.5rem; align-items: center;">
                    <input type="number" 
                           ...
                           placeholder="Shares" 
                           style="width: 100px; padding: 0.5rem; border: 1px solid #E5E5E5;">
                    <button onclick="selectGuarantor('${g.id}', '${g.full_name}', ${g.available_shares})" 
                            class="btn-small">
                        Select
                    </button>
                </div>
...
function updateSelectedGuarantors() {
    ...
    if (selectedGuarantors.length === 0) {
        container.innerHTML = '<p class="text-secondary">No guarantors selected</p>';
        return;
    }

    const totalGuarantorShares = selectedGuarantors.reduce((sum, g) => sum + g.shares, 0);
    
    container.innerHTML = `
        <h4 class="small-title mb-4">Selected Guarantors (${selectedGuarantors.length})</h4>
            ...
            ${selectedGuarantors.map(g => `
                <div class="card" style="padding: 1rem; margin-bottom: 0.5rem; display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        ...
                        <p class="text-secondary" style="font-size: 0.875rem;">
                            ${g.shares} shares (KSh ${(g.shares * 1000).toLocaleString()})
                        </p>
                    </div>
                    <button onclick="removeGuarantor('${g.id}')" class="btn-small" style="background: #EF4444;">
                        Remove
                    </button>
                </div>
            `).join('')}
        ...
        <div style="padding: 1rem; background: #FAFAFA; border: 1px solid #E5E5E5;">
            ...
            <p style="font-weight: 600; font-size: 1.125rem;">
                ${totalGuarantorShares} shares (KSh ${(totalGuarantorShares * 1000).toLocaleString()})
            </p>
        </div>
    `;
}
...
// Close modal
document.getElementById('closeGuarantorModal').addEventListener('click', function() {
    document.getElementById('guarantorModal').style.display = 'none';
});

document.getElementById('guarantorModal').addEventListener('click', function(e) {
    if (e.target === this) {
...
document.getElementById('loanRequestForm').addEventListener('submit', async function(e) {
    ...
    if (amount > maxLoanWithoutGuarantors) {
        const totalGuarantorShares = selectedGuarantors.reduce((sum, g) => sum + g.shares, 0);
        const totalCoverage = (totalShares * 1000) + (totalGuarantorShares * 1000);

        if (totalCoverage < amount) {
            alert(`Insufficient coverage. You need KSh ${(amount - totalCoverage).toLocaleString()} more in guarantor shares.`);
            return;
        }
    }
