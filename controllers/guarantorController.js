...
exports.respondToRequest = async (req, res) => {
    try {
        ...
        if (hasUnpaidLoan) {
            return res.status(400).json({ error: 'You cannot guarantee loans while you have an unpaid loan' });
        }
...
exports.sendRequest = async (req, res) => {
    try {
        ...
        if (loan.status !== 'pending') {
            return res.status(400).json({ error: 'Can only request guarantors for pending loans' });
        }

        // Check if guarantor is eligible
        const hasUnpaidLoan = await User.hasUnpaidLoan(guarantor_id);
        if (hasUnpaidLoan) {
            return res.status(400).json({ error: 'This member has an unpaid loan and cannot be a guarantor' });
        }

        const availableShares = await Share.getAvailableByUser(guarantor_id);
        if (shares_requested > availableShares) {
            return res.status(400).json({ error: 'Guarantor does not have enough available shares' });
        }
