...
class PaymentAllocationService {
  ...
  static async allocatePayment(userId, totalAmount, transactionRef) {
    ...
    try {
      ...
      for (const loan of activeLoans) {
        ...
        const loanPrincipal = loanBalance / 1.03; // Remove 3% interest
      ...
      // 2. PRIORITY: Welfare
      const welfareAmount = paymentConfig.WELFARE_AMOUNT;
      ...
      // 3. PRIORITY: Shares (only until minimum threshold reached)
      const sharePrice = paymentConfig.SHARE_PRICE;
      const currentShares = await Share.getTotalByUser(userId);
      const maxShares = paymentConfig.MAX_SHARES;
      const hasMinimumShares = paymentConfig.hasMinimumShares(currentShares);

      // Only allocate to shares if below minimum threshold
      ...
      // If minimum shares reached, skip to savings (allocation.remaining unchanged)
  ...
  static async getAllocationSummary(userId) {
    ...
    const loanBalance = activeLoans.reduce(
      (sum, loan) => sum + parseFloat(loan.balance_remaining),
      0
    );

    const hasMinimumShares = paymentConfig.hasMinimumShares(totalShares);
    const sharesNeeded = paymentConfig.sharesNeededForMinimum(totalShares);
    const shareValue = totalShares * paymentConfig.SHARE_PRICE;
    ...
    return {
      ...
      min_share_capital: paymentConfig.MIN_SHARE_CAPITAL,
      has_minimum_shares: hasMinimumShares,
      shares_needed: sharesNeeded,
      max_shares: paymentConfig.MAX_SHARES,
      can_buy_shares: !hasMinimumShares && totalShares < paymentConfig.MAX_SHARES,
      personal_savings: personalSavings,
      next_priority:
        loanBalance > 0
          ? "Loan Repayment"
          : !hasMinimumShares
            ? `Share Purchase (${sharesNeeded} more needed)`
            : "Personal Savings",
    };
  }
}
